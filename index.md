...
5. **crypto_DBSCAN**
   - This model was chosen to support the next phase of our research, which explores unsupervised learning techniques as a complement to our supervised classifiers. We selected DBSCAN (Density-Based Spatial Clustering of Applications with Noise) as our primary clustering algorithm due to its strength in identifying clusters of arbitrary shape without requiring the number of clusters to be specified in advance. This is particularly valuable when analyzing noisy and informal data such as tweets in the cryptocurrency domain, where linguistic patterns and sentiment expressions vary widely. Unlike methods like K-means, which assume spherical clusters and require a predefined cluster count, DBSCAN is better suited for uncovering the natural structure in our dataset, including the ability to isolate outliers (often critical in volatile), sentiment-driven environments like crypto discussions (Misingo, 2022)[[9]](#9). We plan to embed tweets using Sentence Transformers and apply dimensionality reduction before clustering to ensure meaningful results. By mapping the resulting clusters to sentiment categories, we aim to uncover latent structures and topic-based groupings that may not be explicitly learned by supervised models. DBSCAN was selected not only for its flexibility and robustness to noise, but also as a baseline for comparison with other clustering methods such as K-means and BIRCH, which we will also evaluate for efficiency and scalability.
